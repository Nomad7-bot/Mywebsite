      ...
      <Link href={`/blog/${article.id}`} className="block">
        ...
        <div className={`space-y-3 ${featured ? 'max-w-xl space-y-4' : ''}`}>
          <div className="flex flex-col gap-1">
            <span className="font-label text-[10px] uppercase tracking-widest text-primary font-extrabold">{article.category}</span>
            <span className="text-[10px] text-on-surface/40 uppercase tracking-widest">{formatDate(article.date)}</span>
          </div>
